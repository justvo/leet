...
var minSubArrayLen = function (target, nums) {
    ...
    if (nums.some((el) => el >= target)) return 1;
    let sum = nums.reduce((cur, acc) => {
      return acc + cur;
    }, 0);
    if (sum < target) return 0;
    if (sum === target) return nl;
    let subsum = 0;
    let start = 0;
    let minLength = Infinity;
    for(let end = 0; end<nl; end++ ){
        subsum += nums[end];
        while(subsum>=target){
...
//   return Math.min(mindminL(nums, nl), mindminL(nums.reverse(), nl));

};

function mindminL(nums, nl) {
  let minLength = nl;
  let left = 0;
  let right = nl;
  let iftoRight = true;
  let iftoLeft = false;
  while (right > 0) {
    const subString = nums.slice(left, right);
    const subSum = subString.reduce((cur, acc) => {
      return acc + cur;
    }, 0);
    if (subSum >= target) {
      if (subString.length === 2) return 2;
      minLength = Math.min(minLength, subString.length);
    }
    if (subSum < target && iftoRight) {
      iftoRight = false;
      iftoLeft = true;
    }
